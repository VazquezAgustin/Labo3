...
int recibir_mensaje(int id_cola_mensajes, long rLongDest, mensaje *rMsg, int bloqueante)
{
    mensaje msg;
    int res;
    res = msgrcv(id_cola_mensajes, (struct msgbuf *)&msg, sizeof(msg.int_rte) + sizeof(msg.int_evento) /*+ sizeof(msg.nro_cuenta) + sizeof(msg.monto) + sizeof(msg.char_mensaje)*/, rLongDest, bloqueante); // 0 bloqueante -  1 no bloqueante
